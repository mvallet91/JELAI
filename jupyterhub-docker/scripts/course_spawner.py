...
class CourseDockerSpawner(DockerSpawner):
    ...
    async def start(self, *args, **kwargs):
        ...
        self.log.info(f"[DEBUG] user_options: {self.user_options}")
        if self.course_id:
            self.log.info(f"[DEBUG] Spawning container for user {self.user.name} for course {self.course_id}")
            self.environment['SELECTED_COURSE_ID'] = self.course_id
            self.log.info(f"[DEBUG] Set SELECTED_COURSE_ID={self.course_id} in environment")
        else:
            self.log.warning(f"[DEBUG] No course_id found in user_options for user {self.user.name}")
