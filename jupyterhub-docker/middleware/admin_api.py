...
@app.put("/api/prompts/{prompt_type}", dependencies=[Depends(require_admin_user)])
...
@app.post("/update-prompt", dependencies=[Depends(require_admin_user)])
...
@app.put("/api/learning-objectives/{task_name}", dependencies=[Depends(require_admin_user)])
...
@app.post("/api/reload-learning-objectives", dependencies=[Depends(require_admin_user)])
@app.post("/api/reload-learning-objectives", dependencies=[Depends(require_admin_user)])
...
@app.get("/api/materials", dependencies=[Depends(require_admin_user)])
...
@app.post("/api/materials", dependencies=[Depends(require_admin_user)])
...
@app.put("/api/experiments", dependencies=[Depends(require_admin_user)])
...
async def get_course_analytics(course_id: str, request: Request):
    """Get student activity analytics for a specific course."""
    user = get_user_from_request(request)
...
async def api_get_course(course_id: str, request: Request):
    """Get a single course by ID, visible to admins, teachers of the course,
    ...
    is_student = user in course.get('students', [])
    if not (is_admin_user(user) or is_teacher_of(course, user) or is_student):
        raise HTTPException(status_code=403, detail='insufficient privileges')
    return course
...
@app.get("/api/materials/{filename}", dependencies=[Depends(require_admin_user)])
...
@app.get("/api/workspace-templates", dependencies=[Depends(require_admin_user)])
...
@app.post("/api/workspace-templates", dependencies=[Depends(require_admin_user)])
...
@app.delete("/api/workspace-templates/{filename}", dependencies=[Depends(require_admin_user)])
...
@app.get("/api/shared-resources", dependencies=[Depends(require_admin_user)])
...
@app.post("/api/shared-resources", dependencies=[Depends(require_admin_user)])
...
@app.delete("/api/shared-resources/{filename}", dependencies=[Depends(require_admin_user)])
