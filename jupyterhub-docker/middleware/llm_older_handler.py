...
# from langchain.utilities import trim_messages
...
model = Ollama(model="phi3.5", base_url=base_url)
...
class InputChat(BaseModel):
    ...
    human_input: str = Field(
        ...
        json_schema_extra={"widget": {"type": "chat", "input": "human_input"}},
    )
    context: Optional[Dict] = Field(
        default=None,
        description="Additional context from the user's notebook actions.",
    )
...
#     input_messages_key="human_input",
#     history_messages_key="history",
# ).with_types(input_type=InputChat, output_type=dict) 
